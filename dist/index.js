...
const fetchSonarMetrics = async (component, metricKeys) => {
    const sonarUrl = (0, core_1.getInput)('sonar-base-url');
    const sonarResponse = await axios_1.default.get(`${sonarUrl}/api/measures/component?component=${component}&metricKeys=${metricKeys}`);
    console.log(JSON.stringify(sonarResponse.data, null, 2));
    return sonarResponse;
};
const submitDDMetrics = async (sonarComponent) => {
    ...
    const pattern = /[-$.+#)]/g;
    const payloadSeries = [];
    sonarComponent.component?.measures.forEach((record) => {
        payloadSeries.push({
    ...
    const ddResponse = await axios_1.default.post(`${ddUrl}/api/v2/series`, data, {
        'headers': {
